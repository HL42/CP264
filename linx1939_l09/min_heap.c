...
static void heapify_up(min_heap *source)
{
    ...
    while (index > 0)
    {
        ...
        {
            break; // break if parent is greater than child
        }
...
static void heapify_down(min_heap *source)
{
    ...
    while (1)
    {
        ...
        int smallest = index;      // index of smallest value
...
int min_heap_valid(const min_heap *source)
{
    ...
    for (int i = 0; i < source->count; i++)
    {
        ...
        if (left < source->count && source->values[left] < source->values[i])
        {
            return 0;
        }

        if (right < source->count && source->values[right] < source->values[i])
        {
            return 0;
        }
    }
